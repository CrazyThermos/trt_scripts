...
def main(onnx_file_path, engine_file_path="", shape=None, build_model=False):
    """Create a TensorRT engine for ONNX-based YOLOv3-608 and run inference."""

    # Try to load a previously generated YOLOv3-608 network graph in ONNX format:
    # Do inference with TensorRT
    trt_outputs = []
    with get_engine(onnx_file_path, engine_file_path, shape, build_model) as engine, engine.create_execution_context() as context:
        ...
        trt_outputs = np.transpose(trt_outputs[0].reshape(3,768,1024),(1,2,0))
...
if __name__ == "__main__":
    ...
    engine_file_path = "./engine/fusion_one_input_fp32_768x1024.engine"
    # engine_file_path = "t1ven.trt"
    shape = [1,3,608,1088]
    main(onnx_file_path, engine_file_path, shape, build_model=False)